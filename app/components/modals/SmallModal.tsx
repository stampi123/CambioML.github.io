...
const SmallModal = ({ isOpen, onClose, title, body, footer }: SmallModalProps) => {
  const [showSmallModal, setShowSmallModal] = useState(isOpen);
  const thisRef = useOutsideClickModal(() => {
    handleClose();
  });

  useEffect(() => {
    setShowSmallModal(isOpen);
  }, [isOpen]);
    ...
    >
      ...
      >
        <div
          ...
          duration-300
          h-full
          ${showSmallModal ? 'translate-y-0' : 'translate-y-full'}
          ${showSmallModal ? 'opacity-100' : 'opacity-0'}
          `}
        >
          <div
            className="
