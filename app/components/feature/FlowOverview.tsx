    ...
    <div className="flex flex-col items-center align-center w-full">
      <div className="h-[400px] grid grid-cols-[200px_105px_200px_80px_275px_120px_140px] w-[1115px] py-5">
        ...
        <div className={`col-span-1 flex items-center justify-center`}>
          <div
            className={`cursor-pointer text-2xl p-3 rounded-lg ${tab === STATE.UNIFLOW ? selectedStyles : unselectedStyles}`}
            onClick={(e) => handleClick(e, STATE.UNIFLOW)}
          >
            Extract
          </div>
        </div>
        ...
        <div className={`col-span-1 flex items-center justify-center`}>
          <div
            className={`cursor-pointer text-2xl p-3 rounded-lg ${tab === STATE.PYKOI ? selectedStyles : unselectedStyles}`}
            onClick={(e) => handleClick(e, STATE.PYKOI)}
          >
            Train
          </div>
        </div>
        ...
        <div className={`col-span-1 flex items-center justify-center`}>
          <div
            className={`cursor-pointer text-2xl p-3 rounded-lg ${tab === STATE.AUTOEVAL ? selectedStyles : unselectedStyles}`}
            onClick={(e) => handleClick(e, STATE.AUTOEVAL)}
          >
            Validate
          </div>
        </div>
      ...
      <div className="lg:h-[600px] w-full py-5 rounded-lg">
        {tab === STATE.UNIFLOW && (
          <CodeBlock
            code={`from uniflow import ExtractMarkdowClient
...
output = client.run(data)`}
        ...
        {tab === STATE.PYKOI && (
          <DemoTab
            code={`from pykoi import Finetuning, RLHFConfig
...
rlhf.train_and_save("./models")`}
        ...
        {tab === STATE.AUTOEVAL && (
          <DemoTab
            code={`from uniflow import AutoRater
...
const FlowImage = ({ image, alt }: FlowImageProps) => {
  return (
